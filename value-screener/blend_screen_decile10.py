...
return (+28% more excess) at the cost of concentration and 60d reliability.

    score = rank(idiosyncratic momentum 12-1)
          + rank(price vs 200-day MA)
          + rank(golden cross: SMA50/SMA200 spread + freshness)

    positions are RANK-WEIGHTED within the top 10% (weight proportional to
...
a FILTER: only names with SMA50 > SMA200 are eligible (~357 of ~507); the top 10%
selection then holds ~35 names, rank-weighted with the top position ~5.6%.

QUICK START — no API key, no third-party packages, Python 3.9+:

    python3 blend_screen_decile10.py                 # top 25, 60-day hold
    python3 blend_screen_decile10.py --top 50
    python3 blend_screen_decile10.py --hold 30
    python3 blend_screen_decile10.py --refresh       # re-download prices
    python3 blend_screen_decile10.py --csv picks.csv # export the ranking
...
Held out on 2024-2026, one side-by-side pipeline:
    ...
    THIS SCREEN (10%)   +4.386%  3.99   +8.98%  62.2%   2.28  | +4.390%  4.12  +18.10%  66.5%   2.53
    5% sibling          +5.604%  4.17  +10.71%  63.4%   2.27  | +5.624%  3.83  +21.61%  67.9%   2.41
...
DEFAULT_DECILE = 0.10    # this file's identity; the 5% sibling is blend_screen_standalone.py
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    for sym, path in fetched.items():
        ...
        if sig[-1]["golden_cross"] is None:
            skipped_no_cross += 1   # counted for display; rank_composite ranks the
                                    # momentum legs over ALL names (validated
                                    # construction) and the missing gc leg filters
        latest[sym] = sig[-1]
    ...
    print("  Construction: top 10% of eligible names, rank-weighted (the weight")
    print("  column) — the best 60d t (4.12) and Sharpe (2.53) of the blend variants.")
    print("  The 5% sibling (blend_screen_standalone.py) trades that reliability for")
    print("  ~+20% more raw return per trade with ~18 names and an 11% top position.")
