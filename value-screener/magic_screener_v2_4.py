#!/usr/bin/env python3
"""AEGIS Magic Screener v2.4 — evidence-oriented fundamental + technical entry engine.
...
v2.4 corrections
----------------
- Backtest barriers are volatility-scaled by default: the profit target and stop
  are multiples of ATR% measured at the signal date, so names are tested at the
  same distance in units of their own noise. Fixed-percentage barriers measured
  volatility rather than signal — a 3% stop is ~3 ATR on a 1%-ATR name and
  ~0.4 ATR on a 7%-ATR name, stopping the latter out essentially at random, and
  flipping the sign of reported expectancy purely on the barrier choice.
  ``--bt-barrier-mode pct`` restores the v2.3 behaviour for reproducing old runs.
- MFE/MAE no longer credit excursions that happen after the position is closed.
  v2.3 updated them from each bar's full high/low *before* testing the exit, so
  a trade stopped out at -3% could report a +30% maximum favourable excursion.
- Sharpe/Sortino are annualized on realized trade frequency instead of
  ``252 / avg_holding_days``. The old convention assumed capital was invested
  continuously and inflated the ratio by roughly ``sqrt(1 / time_in_market)``
  for a strategy that is flat between non-overlapping setups. ``timeInMarketPct``
  and ``tradesPerYear`` are now reported so the assumption is visible.
- Technical coverage is a real measurement. Every component previously returned
  a fabricated neutral 50 when its inputs were missing, so coverage was only
  ever 86% or 100% and the ``coverage >= 70`` setup gate could never fail.
  Components now return None when unobserved, and ``weeklyTrend`` is None rather
  than "neutral" when there is not enough weekly history to judge.
...
python magic_screener_v2_4.py NVDA --history-csv nvda.csv

# Add benchmark-relative strength:
python magic_screener_v2_4.py NVDA --history-csv nvda.csv --benchmark-csv spy.csv

# Optional anchored VWAP from an earnings/catalyst date:
python magic_screener_v2_4.py NVDA --history-csv nvda.csv --anchor-date 2026-05-20
...
export FMP_API_KEY='...'
python magic_screener_v2_4.py NVDA --live

# Walk-forward validation of the entry model:
python magic_screener_v2_4.py NVDA --history-csv nvda.csv --backtest \
    --bt-horizon 20 --bt-profit 0.05 --bt-stop 0.03 --bt-min-score 75

# Export one ML-ready feature row:
python magic_screener_v2_4.py NVDA --history-csv nvda.csv --features-json nvda_features.json
...
def _get_json(url: str) -> Any:
    req = urllib.request.Request(url, headers={"User-Agent": "AEGIS-Magic-Screener/2.4"})
...
def compute_technical_features(
    ...
    # None means "not enough weekly history to judge", which is materially
    # different from an observed neutral and must not score as half a point.
...
def _trend_component(f: dict[str, Any]) -> Optional[float]:
    p = f["price"]
    points = 0.0
    possible = 0.0
    total = 0.0
    ...
    for ma, w in checks:
        ...
        if ma is not None:
            ...
            if p > ma:
                points += w
    total += 15
    if f.get("sma20") is not None and f.get("sma50") is not None and f.get("sma200") is not None:
        possible += 15
    ...
    for key, w in (("sma20Slope10d", 8), ("sma50Slope10d", 10), ("sma200Slope20d", 7)):
        ...
        if v is not None:
            ...
            if v > 0:
                points += w
    total += 20
    if f.get("weeklyTrend") is not None:
        possible += 20
        points += 20 if f["weeklyTrend"] == "bull" else 0 if f["weeklyTrend"] == "bear" else 10
    # On a short history most of this evidence is missing. Report the absence so
    # it lands in the coverage figure instead of returning a fabricated neutral.
    if possible < 0.5 * total:
        return None
    return points / possible * 100.0


def _breakout_component(f: dict[str, Any]) -> Optional[float]:
    if f.get("breakout20") is None and f.get("breakout55") is None:
        return None
    score = 25.0
    if f.get("breakout20"):
        score += 25
...
def _momentum_component(f: dict[str, Any]) -> Optional[float]:
...
def _volume_component(f: dict[str, Any]) -> Optional[float]:
    rv = f.get("relVol20")
    vz = f.get("volumeZ20")
    if rv is None:
        return None
...
def _vwap_component(f: dict[str, Any]) -> Optional[float]:
    if f.get("aboveVwap20") is None and f.get("aboveAnchoredVwap") is None:
        return None
    score = 50.0
...
def _oscillator_component(f: dict[str, Any]) -> Optional[float]:
    rsi = f.get("rsi14")
    hist = f.get("macdHist")
    accel = f.get("macdHistAccel")
    if rsi is None and hist is None and accel is None:
        return None
    score = 50.0
    if rsi is not None:
        if 50 <= rsi <= 68:
...
def _path_outcome(
    ...
    def excursion(level: float) -> None:
        """Credit a price the trade actually experienced while still open."""
        nonlocal mfe, mae
        r = level / entry - 1.0
        mfe = max(mfe, r)
        mae = min(mae, r)

    # Excursions are only credited up to the point the position is closed.
    # Crediting the exit bar's full high/low would report favourable and adverse
    # moves that happened after the trade was already out of the market.
    for i, b in enumerate(future, 1):
        # Opening gaps happen before the intraday high/low path is observed,
        # so a gap exit experiences the open and nothing else on that bar.
        if b.open <= stop:
            excursion(b.open)
        ...
        # the stop is assumed to trigger first, so this bar cannot also be
        # credited with its high (nor the target bar with its low).
        if b.low <= stop:
            excursion(b.open)
            excursion(stop)
            outcome, hit_day = "STOP", i
        ...
        # The position survived the whole bar, so its full range was experienced.
        excursion(b.high)
        excursion(b.low)
...
def _sharpe_sortino_approx(returns: list[float], trades_per_year: Optional[float]) -> tuple[Optional[float], Optional[float]]:
    """Annualize per-trade returns by the realized trade *frequency*.

    Annualizing by ``252 / avg_holding_days`` — as v2.3 did — assumes capital is
    invested continuously. Non-overlapping setups leave the account flat between
    trades, so that convention scales a handful of trades as if they were a
    year's worth of them and inflates the ratio by roughly
    ``sqrt(1 / time_in_market)``. Using trades per year charges the strategy for
    the idle capital it actually holds.
    """
    if len(returns) < 2 or not trades_per_year or trades_per_year <= 0:
        return None, None
    mean_r = statistics.fmean(returns)
    sd = safe_stdev(returns)
    sharpe = None if sd in (None, 0) else mean_r / sd * math.sqrt(trades_per_year)
    downside = [min(r, 0.0) ** 2 for r in returns]
    dd = math.sqrt(statistics.fmean(downside)) if downside else 0.0
    sortino = None if dd == 0 else mean_r / dd * math.sqrt(trades_per_year)
...
# Volatility-scaled barriers are clamped into a sane band so a near-zero-ATR
# quote stub cannot produce a 0.1% stop, and a halted//gapping name cannot
# produce a barrier so wide that the horizon exit is the only possible outcome.
MIN_BARRIER_FRACTION = 0.005
MAX_BARRIER_FRACTION = 0.50
...
def _resolve_barriers(
    ...
    """Per-signal profit/stop fractions.

    In ``atr`` mode the barriers are a fixed multiple of the volatility observed
    at the signal date, so a 1%-ATR utility and a 7%-ATR small cap are both
    tested at the same distance *in units of their own noise*. Fixed-percentage
    barriers measure volatility, not signal: a 3% stop is ~3 ATR on the former
    and ~0.4 ATR on the latter, which stops the latter out essentially at random.
    """
    if barrier_mode == "pct":
        return profit_target, stop_loss
    if atr_pct is None or atr_pct <= 0 or not math.isfinite(atr_pct):
        return None
    target = clamp(atr_target_mult * atr_pct / 100.0, MIN_BARRIER_FRACTION, MAX_BARRIER_FRACTION)
    stop = clamp(atr_stop_mult * atr_pct / 100.0, MIN_BARRIER_FRACTION, MAX_BARRIER_FRACTION)
    return target, stop
...
def backtest_entry_model(
    ...
    barrier_mode='atr' (default) sizes the profit target and stop as multiples
    of ATR% measured at the signal date; 'pct' restores the v2.3 fixed-percentage
    behaviour and is kept only for reproducing older runs.
    """
    if mode not in {"trade", "signal"}:
        raise ValueError("mode must be 'trade' or 'signal'")
    if barrier_mode not in {"atr", "pct"}:
        raise ValueError("barrier_mode must be 'atr' or 'pct'")
    ...
    if barrier_mode == "atr" and (atr_target_mult <= 0 or atr_stop_mult <= 0):
        raise ValueError("atr_target_mult and atr_stop_mult must be positive")
    ...
    for i in range(warmup, len(bars) - horizon):
        ...
        # ATR% is measured on `hist`, which ends at the signal bar T, while the
        # entry is bar T+1's open — the barriers therefore carry no look-ahead.
        atr_pct_at_signal = f.get("atrPct")
        barriers = _resolve_barriers(
            atr_pct_at_signal, barrier_mode, atr_target_mult, atr_stop_mult,
            profit_target, stop_loss,
        )
        if barriers is None:
            continue
        target_frac, stop_frac = barriers
        ...
        cal_path = _path_outcome(
            full_future, entry_price, target_frac, stop_frac,
        ...
        signals.append({
            ...
            "setup": setup,
            "atrPctAtSignal": atr_pct_at_signal,
            "targetPct": target_frac * 100.0,
            "stopPct": stop_frac * 100.0,
    ...
    if n == 0:
        return {
            ...
            "atrTargetMult": atr_target_mult if barrier_mode == "atr" else None,
            "atrStopMult": atr_stop_mult if barrier_mode == "atr" else None,
            "signals": 0,
            "tradesPerYear": 0.0,
            "timeInMarketPct": 0.0,
    ...
    # The strategy was "live" across every evaluated signal day, whether or not
    # it held a position, so that window is the denominator for both annualization
    # and time-in-market.
    span_bars = max(1, (len(bars) - horizon) - warmup)
    span_years = span_bars / 252.0
    trades_per_year = n / span_years if span_years > 0 else None
    time_in_market = min(1.0, sum(holding) / span_bars) if holding else 0.0
    ...
    if mode == "trade":
        sharpe, sortino = _sharpe_sortino_approx(returns, trades_per_year)
    ...
    return {
        ...
        "atrTargetMult": atr_target_mult if barrier_mode == "atr" else None,
        "atrStopMult": atr_stop_mult if barrier_mode == "atr" else None,
        "avgTargetPct": statistics.fmean(x["targetPct"] for x in signals),
        "avgStopPct": statistics.fmean(x["stopPct"] for x in signals),
        "signals": n,
        ...
        "timeInMarketPct": time_in_market * 100.0,
...
def format_report(r: dict[str, Any]) -> str:
    ...
    lines = [
        f"===== AEGIS MAGIC SCREENER v2.4 — {r['ticker']} ({r['name']}) =====",
        ...
        f"  Weekly trend          : {t['weeklyTrend'] or 'n/a'}  (W10={fmt(t['weeklySma10'])}, W30={fmt(t['weeklySma30'])})",
...
def _barrier_description(bt: dict[str, Any], profit: float, stop: float) -> str:
    if bt.get("barrierMode") == "atr":
        avg_t, avg_s = bt.get("avgTargetPct"), bt.get("avgStopPct")
        realized = (f"; realized avg +{avg_t:.2f}%/-{avg_s:.2f}%"
                    if avg_t is not None and avg_s is not None else "")
        return (f"ATR-scaled: target {fmt(bt.get('atrTargetMult'),2)} x ATR, "
                f"stop {fmt(bt.get('atrStopMult'),2)} x ATR{realized}")
    return f"fixed: target +{profit*100:.1f}%; stop -{stop*100:.1f}%"
...
def format_backtest(bt: dict[str, Any], horizon: int, profit: float, stop: float, min_score: float) -> str:
    ...
    lines = [
        "===== WALK-FORWARD ENTRY BACKTEST v2.4 =====",
        f"Mode                  : {bt['mode']} (trade = non-overlapping)",
        f"Trades/signals        : {bt['signals']}",
        f"Rule                  : EOD setup score >= {min_score:.1f}; execute NEXT OPEN; horizon {horizon}d",
        f"Barriers              : {_barrier_description(bt, profit, stop)}",
        ...
        f"Trades per year       : {fmt(bt.get('tradesPerYear'),2)}",
        f"Time in market        : {fmt(bt.get('timeInMarketPct'),1,'%')}  (Sharpe is annualized on trade frequency, so idle capital counts)",
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="AEGIS Magic Screener v2.4 — real OHLCV EOD setup + validated next-open backtest engine",
    ...
    p.add_argument("--bt-barrier-mode", choices=["atr", "pct"], default="atr",
                   help="atr scales target/stop by volatility at the signal date; pct uses the fixed --bt-profit/--bt-stop")
    p.add_argument("--bt-atr-target", type=float, default=3.0, help="profit target in ATR multiples (atr mode)")
    p.add_argument("--bt-atr-stop", type=float, default=2.0, help="stop loss in ATR multiples (atr mode)")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if args.backtest:
        bt = backtest_entry_model(
            ...
            barrier_mode=args.bt_barrier_mode,
            atr_target_mult=args.bt_atr_target,
            atr_stop_mult=args.bt_atr_stop,
        )
